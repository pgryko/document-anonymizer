...
def main():
    ...
    parser.add_argument('--auto-create-bucket', action='store_true', default=True,
    ...
    if not args.access_key_id:
        print("❌ R2 Access Key ID is required (--access-key-id or CLOUDFLARE_R2_ACCESS_KEY_ID env var)")
        return 1
    if not args.secret_access_key:
        print("❌ R2 Secret Access Key is required (--secret-access-key or CLOUDFLARE_R2_SECRET_ACCESS_KEY env var)")
        return 1
    if not args.endpoint_url:
        print("❌ R2 Endpoint URL is required (--endpoint-url or CLOUDFLARE_R2_ENDPOINT_URL env var)")
        return 1
    ...
    if args.auto_create_bucket and not args.dry_run:
        ...
        if account_id and api_token:
            ...
            print("⚠️  Auto-create bucket requested but CLOUDFLARE_ACCOUNT_ID or CLOUDFLARE_ACCOUNT_API_TOKEN not available")
