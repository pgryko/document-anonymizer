...
class CloudflareR2Manager:
    ...
    def delete_bucket(self, bucket_name: str) -> bool:
        """Delete an R2 bucket."""
        response = self._make_request("DELETE", f"/accounts/{self.account_id}/r2/buckets/{bucket_name}")
    ...
    def delete_token(self, access_key_id: str) -> bool:
        ...
        response = self._make_request("DELETE", f"/accounts/{self.account_id}/r2/credentials/{access_key_id}")
...
def main():
    ...
    parser.add_argument('--api-token', default=os.getenv('CLOUDFLARE_ACCOUNT_API_TOKEN'),
    ...
    create_bucket_parser.add_argument('--location', default='WNAM', 
                                    help='Location hint (WNAM, ENAM, APAC, etc.)')
    ...
    create_token_parser.add_argument('--permissions', nargs='+', 
                                   choices=['admin', 'read', 'edit'],
                                   default=['admin'], help='Token permissions')
    create_token_parser.add_argument('--buckets', nargs='+', 
                                   help='Scope to specific buckets')
    create_token_parser.add_argument('--ttl-days', type=int,
                                   help='Token TTL in days')
    ...
    setup_parser.add_argument('--bucket-name', default='document-anonymizer-data',
                            help='Bucket name to create')
    setup_parser.add_argument('--token-name', default='document-anonymizer-token',
                            help='Token name to create')
    
    # Auto-ensure command
    auto_ensure_parser = subparsers.add_parser('auto-ensure', help='Auto-create bucket from .env if it does not exist')
    auto_ensure_parser.add_argument('--location', default='WNAM', 
    ...
    if not args.api_token:
        print("❌ CLOUDFLARE_ACCOUNT_API_TOKEN is required (set via --api-token or environment variable)")
        sys.exit(1)
