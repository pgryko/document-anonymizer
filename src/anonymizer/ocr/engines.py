                ...
                for line in result[0]:
                    if len(line) >= 2:
                        ...
                        if len(text_info) >= 2:
                            ...
                            if (confidence >= self.config.min_confidence_threshold and
...
class TrOCREngine(BaseOCREngine):
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            self.logger.warning("TrOCR not available - install with: pip install transformers torch")
            return False
        except Exception as e:
    ...
    def detect_text(self, image: np.ndarray) -> OCRResult:
        ...
        try:
            ...
            for region_bbox, region_image in text_regions:
                ...
                # Filter by length
                if self.config.min_text_length <= len(generated_text) <= self.config.max_text_length:
                    detected_text = DetectedText(
                        text=generated_text,
                        bbox=region_bbox,
                        confidence=confidence,
    ...
    def detect_text(self, image: np.ndarray) -> OCRResult:
        ...
        try:
            ...
            for i in range(n_boxes):
                ...
                if (text and confidence >= self.config.min_confidence_threshold * 100 and  # Tesseract uses 0-100 scale
                    self.config.min_text_length <= len(text) <= self.config.max_text_length):
