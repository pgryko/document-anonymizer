...
from .models import DetectedText, OCRResult, OCRConfig, OCREngine, OCRMetrics
...
class OCRProcessor:
    ...
    def initialize(self) -> bool:
        ...
        try:
            ...
            if not primary_success:
                logger.warning(f"Primary engine {self.config.primary_engine.value} failed to initialize")
    ...
    def extract_text_regions(self, image: np.ndarray, 
                           min_confidence: Optional[float] = None) -> List[DetectedText]:
        """
        ...
        if self.primary_engine and self.primary_engine.is_initialized:
            try:
                ...
                if result.success and len(result.detected_texts) > 0:
                    ...
                    logger.warning(f"Primary engine {self.config.primary_engine.value} returned no results")
        ...
        if (result is None or not result.success or len(result.detected_texts) == 0) and self.fallback_engines:
            ...
            for fallback_engine in self.fallback_engines:
                try:
                    ...
                    if result.success and len(result.detected_texts) > 0:
                        ...
                        logger.warning(f"Fallback engine {result.engine_used.value} returned no results")
        ...
        filtered_texts = [
            text for text in result.detected_texts 
        ...
        logger.info(f"OCR completed: {len(filtered_texts)} text regions detected in {processing_time:.2f}s")
        
        return filtered_texts
    
    def convert_to_text_regions(self, detected_texts: List[DetectedText],
                              replacement_strategy: str = "generic") -> List[TextRegion]:
        """
    ...
    def detect_and_convert(self, image: np.ndarray,
                          replacement_strategy: str = "generic") -> List[TextRegion]:
        """
